...
build-release:
	...
	./scripts/package-workflow.sh target/release/alfred_flutter_docs
...
build-universal: build-arm64 build-x86_64
	...
	lipo -create \
		target/aarch64-apple-darwin/release/alfred_flutter_docs \
		target/x86_64-apple-darwin/release/alfred_flutter_docs \
...
package: build-universal
	...
	(cd build/dist && zip -qr "../$${WORKFLOW_NAME}-v$${VERSION}.alfredworkflow.tmp.zip" . \
		-x '.env' 'query_cache/*' 'update_cache/*' '*_cache/*' '*_cache_keys.json' 'workflow_intel' \
		   'create_index.py' 'requirements.txt' 'full_index.json'); \
